  ...
  return (
    <div className="space-y-6">
      <div className="text-center mb-8">
        <h2 className="font-heading font-extrabold text-3xl text-gray-900 mb-2">Select a Listing Package</h2>
        <p className="text-gray-500">Choose the level of exposure you want for your Shelby.</p>
      </div>
        ...
        {PACKAGES.map((pkg) => (
          <div 
            ...
            className={`relative rounded-3xl p-6 border-2 cursor-pointer transition-all duration-300 ${
              selected === pkg.id 
                ? "border-[var(--color-shelby-blue)] bg-blue-50/50 shadow-xl scale-[1.02]" 
                : "border-gray-200 bg-white hover:border-gray-300 shadow-sm"
            }`}
          >
            {pkg.recommended && (
              <div className="absolute -top-4 left-1/2 -translate-x-1/2 bg-[var(--color-shelby-red)] text-white text-xs font-bold uppercase py-1 px-4 rounded-full shadow-md">
                Recommended
              </div>
            )}
            <h3 className="font-heading font-bold text-xl text-gray-900 mb-2">{pkg.name}</h3>
            <div className="font-black text-4xl text-[var(--color-shelby-blue)] mb-4">${pkg.price}</div>
            <p className="text-sm text-gray-600 mb-6 min-h-[40px]">{pkg.desc}</p>
      ...
      <div className="flex justify-between pt-8 border-t border-gray-100 mt-8">
        ...
        <button onClick={() => onNext({ package_tier: selected })} className="px-8 py-4 bg-[var(--color-shelby-blue)] text-white font-bold rounded-xl hover:bg-[#001D40] transition-colors shadow-lg active:scale-95">
